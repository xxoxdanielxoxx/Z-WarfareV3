...
public bool OODCCQOOOQ;
public bool ODQDOQOO;
public float OODDCQQOOD;
public float ODOOQQOO;
public Transform[] ODDODDCCDCs;
public float[] trperc;
...
public string OQCQOCOOCQ = "0";
public string OOCOCCDQOQ = "0";
...
if(!objectScript.ODQDCQQQDO){  


Vector3 v;
if(snapMarker){
if(ODQOOOQOQO.terrain != null){
v = transform.position;
v.y = ODQOOOQOQO.terrain.SampleHeight(v)+ ODQOOOQOQO.terrain.transform.position.y;
transform.position = v;
}
}

Vector3 change = transform.position - oldPos;
if(OODCCQOOOQ && oldPos != Vector3.zero && change != Vector3.zero){
int i = 0;
foreach(Transform tr in ODDODDCCDCs){
tr.position += change * trperc[i];

if(snapMarker){
if(ODQOOOQOQO.terrain != null){
v = tr.position;
v.y = ODQOOOQOQO.terrain.SampleHeight(v);
tr.position = v;
}
}
...
changed = true;
if(objectScript.ODQDCQQQDO){
objectScript.OOCQCQDOQO.specialRoadMaterial = true;
}
}
...
for(int i = 0; i < objectScript.ODDODDCCDCs.Length; i++){
if(objectScript.ODDODDCCDCs[i] == this.gameObject)return true;
}
return false;
}
}
