...
Shader "Shader Forge/glossmap" {
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform sampler2D _GlosTex; uniform float4 _GlosTex_ST;
...
/////// Diffuse:
                ...
                #endif
///////// Gloss:
                float2 node_152 = i.uv0;
                float node_143 = (tex2D(_GlosTex,TRANSFORM_TEX(node_152.rg, _GlosTex)).a*_shine);
                float gloss = node_143;
                float specPow = exp2( gloss * 10.0+1.0);
////// Specular:
                NdotL = max(0.0, NdotL);
                float3 specularColor = pow(tex2D(_SpecTex,TRANSFORM_TEX(node_152.rg, _SpecTex)).rgb,2.0);
                ...
                diffuseLight += (lightmap*marmoDiffuse(UnpackNormal(tex2D(_BumpMap,TRANSFORM_TEX(node_152.rg, _BumpMap))).rgb)).rgb; // Diffuse Ambient Light
                finalColor += diffuseLight * tex2D(_MainTex,TRANSFORM_TEX(node_152.rg, _MainTex)).rgb;
                finalColor += specular;
/// Final Color:
...
///////// Gloss:
                ...
                float node_143 = (tex2D(_GlosTex,TRANSFORM_TEX(node_153.rg, _GlosTex)).a*_shine);
                float gloss = node_143;
                float specPow = exp2( gloss * 10.0+1.0);
////// Specular:
                NdotL = max(0.0, NdotL);
                float3 specularColor = pow(tex2D(_SpecTex,TRANSFORM_TEX(node_153.rg, _SpecTex)).rgb,2.0);
                float3 specular = attenColor * pow(max(0,dot(halfDirection,normalDirection)),specPow) * specularColor;
                float3 finalColor = 0;
                float3 diffuseLight = diffuse;
                finalColor += diffuseLight * tex2D(_MainTex,TRANSFORM_TEX(node_153.rg, _MainTex)).rgb;
                finalColor += specular;
/// Final Color:
